...
<body>
  <h1>🧭 CTFPilot — Reporte de Sesion</h1>
  <h2>Informacion General</h2>
  <table>
    <tr><th>Campo</th><th>Valor</th></tr>
    <tr><td>Maquina</td><td>{{ session.name }}</td></tr>
  ...
  {% endif %}
  <div class="footer">Generado por CTFPilot v0.1.0 — {{ generated_at }}</div>
</body>
</html>
"""
...
def generate_markdown(session_id: int) -> str:
    ...
    md = f"""# {name} — CTFPilot Writeup

> **Plataforma:** {platform.upper()} | **Target:** `{target}` | **Fecha:** {started_at[:10]}

---

## Reconocimiento

> Añade aqui los resultados del reconocimiento

## Enumeracion

> Añade aqui los hallazgos de enumeracion

## Explotacion

> Documenta aqui el proceso de explotacion

## Post-Explotacion

> Documenta aqui las acciones post-explotacion

## Flags

"""
    for flag in flags:
        md += f"- **[{flag['type'].upper()}]** `{flag['value']}`\n"
    if not flags:
        md += "- No se registraron flags.\n"

    md += "\n## Notas de Sesion\n\n"
    for note in notes:
        md += f"- {note['content']} _{note['created_at'][:16]}_\n"
    if not notes:
        md += "- No se registraron notas.\n"
